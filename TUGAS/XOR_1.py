...
def maju():
    ...
    try:
        while True:
            ...
            Z_net1 = float(bi[0]) + (float(x1[0])*float(Vij[0])) + (float(x2[0])*float(Vij[3]))
            Z_net2 = float(bi[1]) + (float(x1[0])*float(Vij[1])) + (float(x2[0])*float(Vij[4]))
            Z_net3 = float(bi[2]) + (float(x1[0])*float(Vij[2])) + (float(x2[0])*float(Vij[5]))
